...
var flattenAllOfProperties = function (schemaDefinition) {
    var parentSchema = JSON.parse(JSON.stringify(schemaDefinition.allOf[0]));
    var additionalSchema = schemaDefinition.allOf[1] || {};
    var finalSchema = parentSchema;
    for (var key in additionalSchema) {
        // If it's an object, we need to merge in all properties.
        if (additionalSchema[key] === Object(additionalSchema[key])) {
            for (var propertyKey in additionalSchema[key]) {
